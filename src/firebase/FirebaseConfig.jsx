...
import { getFirestore ,collection, addDoc,getDocs,getDoc,doc,setDoc,updateDoc,query,where,onSnapshot, deleteField, arrayRemove, arrayUnion} from "firebase/firestore";
...
export const signInWithGoogle = async () => {
  try {
    ...
    console.error('Error signing in: ', error.code, error.message, error.customData.email, GoogleAuthProvider.credentialFromError(error));
    throw error;
  }
};
...
export const getCourseDetails = async (courseIds) => {
  ...
  try {
    ...
    const coursesData = coursesSnapshots.map(snapshot => ({
      id: snapshot.id,
      ...snapshot.data(),
      completed: courseIds.find(courseObj => courseObj.id === snapshot.id).completed
    }));
    return coursesData;
  } catch (error) {
    console.error('Error fetching courses:', error);
...
//       courses: deleteField(courseId) // Remove the courseId from the 'courses' array
//     });
...
// export const addPurchasedCourses = async (userId, courseIds) => {
//   if (!Array.isArray(courseIds) || courseIds.includes(undefined)) {
//     throw new Error("Invalid courseIds array");
//   }
...
//       await setDoc(userCoursesDocRef, {
//         courses: courseIds,
//       });
//     }
//   } catch (error) {
...
// export const addPurchasedCourses = async (userId, courses) => {
//   if (!Array.isArray(courses) || courses.some(course => !course.id)) {
...
export const addPurchasedCourses = async (userId, newCourses) => {
  ...
  try {
    ...
    if (userCoursesDocSnap.exists()) {
      ...
      const existingCoursesMap = new Map(existingCourses.map(course => [course.id, course]));
      newCourses.forEach(course => existingCoursesMap.set(course.id, { ...course, completed: false }));
